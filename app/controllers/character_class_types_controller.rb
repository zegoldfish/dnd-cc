class CharacterClassTypesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @character_class_type.save
        ...
        format.json { render action: 'show', status: :created, location: @character_class_type }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to character_class_types_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def character_class_type_params
      params[:character_class_type].permit(:character_class_name)
    end
end
